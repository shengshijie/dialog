...
class AlertDialogImpl(context: Context) : IAlertDialog {
    ...
    override fun setPositiveButton(text: String?, f: (IAlertDialog) -> Unit) {
        alertDialog.setButton(BUTTON_POSITIVE, text) { _, _ -> f.invoke(this@AlertDialogImpl) }
    }

    override fun setNegativeButton(text: String?, f: (IAlertDialog) -> Unit) {
        alertDialog.setButton(BUTTON_NEGATIVE, text) { _, _ -> f.invoke(this@AlertDialogImpl) }
    }

    override fun setOnDismiss(f: (IDialog) -> Unit) {
        alertDialog.setOnDismissListener { f.invoke(this@AlertDialogImpl) }
    }
