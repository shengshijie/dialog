...
class InputDialogImpl(context: Context) : IInputDialog {
    ...
    override fun setPositiveButton(text: String?, f: (IInputDialog, text: String?) -> Unit) {
        alertDialog.setButton(BUTTON_POSITIVE, text) { _, _ -> f.invoke(this@InputDialogImpl, editText.text.toString()) }
    }

    override fun setNegativeButton(text: String?, f: (IInputDialog, text: String?) -> Unit) {
        alertDialog.setButton(BUTTON_NEGATIVE, text) { _, _ -> f.invoke(this@InputDialogImpl, editText.text.toString()) }
    }

    override fun setOnDismiss(f: (IDialog) -> Unit) {
        alertDialog.setOnDismissListener { f.invoke(this@InputDialogImpl) }
    }
